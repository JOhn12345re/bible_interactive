...
const BibleQuizGame = () => {
  ...
  const questions: Question[] = [
    {
      ...
      correctAnswer: 2,
      explanation: "Noé a construit l'arche sur ordre de Dieu pour sauver sa famille et les animaux du déluge.",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Jésus a jeûné pendant 40 jours et 40 nuits dans le désert avant d'être tenté par Satan.",
    ...
    },
    {
      ...
      correctAnswer: 2,
      explanation: "David a utilisé une fronde et une pierre pour vaincre le géant Goliath.",
    ...
    },
    {
      ...
      options: ["10", "12", "7", "15"],
      correctAnswer: 1,
      explanation: "Jésus a choisi douze apôtres pour l'accompagner et répandre son message.",
    ...
    },
    {
      ...
      correctAnswer: 2,
      explanation: "Jésus est né à Bethléem, dans une étable, car il n'y avait pas de place dans l'hôtellerie.",
    ...
    },
    {
      ...
      correctAnswer: 2,
      explanation: "Judas Iscariote a trahi Jésus en le livrant aux autorités pour 30 pièces d'argent.",
    ...
    },
    {
      ...
      options: ["7", "10", "12", "5"],
      correctAnswer: 1,
      explanation: "Dieu a envoyé dix plaies sur l'Égypte pour convaincre Pharaon de libérer le peuple d'Israël.",
  ...
  const handleNextQuestion = () => {
    if (currentQuestion + 1 < questions.length) {
      ...
      if (percentage >= 80) {
        addFavoriteVerse("Car Dieu a tant aimé le monde qu'il a donné son Fils unique - Jean 3:16");
      }
      if (percentage === 100) {
        addFavoriteVerse("Toute Écriture est inspirée de Dieu, et utile - 2 Timothée 3:16");
      }
    }
  };
  ...
  if (gameFinished) {
    return (
      <div className={`min-h-screen ${contrastHigh ? 'bg-contrast-bg text-contrast-text' : 'bg-gradient-to-br from-blue-50 via-white to-indigo-50'}`}>
          ...
          <div className="max-w-2xl mx-auto">
            <div className={`p-8 rounded-2xl text-center ${
              ...
              <p className={`text-lg mb-8 ${contrastHigh ? 'text-contrast-text' : 'text-gray-600'}`}>
                {score === questions.length && "Parfait ! Tu connais très bien la Bible ! 🌟"}
                {score >= questions.length * 0.8 && score < questions.length && "Excellent ! Tu as une très bonne connaissance de la Bible ! 👏"}
                {score >= questions.length * 0.6 && score < questions.length * 0.8 && "Bien joué ! Continue à étudier la Bible ! 📖"}
                {score < questions.length * 0.6 && "Continue à lire la Bible pour apprendre davantage ! 💪"}
              </p>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-right">
            <div className={`text-sm ${contrastHigh ? 'text-contrast-text' : 'text-gray-600'}`}>
              Question {currentQuestion + 1}/{questions.length}
            </div>
            <div className={`text-lg font-bold ${contrastHigh ? 'text-contrast-text' : 'text-blue-600'}`}>
              Score: {score}
            </div>
          </div>
        </div>
        ...
        {/* Progress Bar */}
        <div className={`w-full h-2 rounded-full mb-8 ${contrastHigh ? 'bg-contrast-text/20' : 'bg-gray-200'}`}>
          <div 
          ...
          <div className={`p-8 rounded-2xl ${
            ...
            {showResult && (
              <div className={`p-6 rounded-xl mb-6 ${
                ...
                <div className="flex items-start space-x-3">
                  <span className="text-2xl">💡</span>
                  <div>
                    <h4 className={`font-bold mb-2 ${contrastHigh ? 'text-contrast-text' : 'text-blue-800'}`}>
                      Explication
                    </h4>
                    <p className={`mb-3 ${contrastHigh ? 'text-contrast-text' : 'text-blue-700'}`}>
                      {currentQ.explanation}
                    </p>
                    <p className={`text-sm font-medium ${contrastHigh ? 'text-contrast-text/80' : 'text-blue-600'}`}>
                      📖 {currentQ.reference}
                    </p>
                  </div>
                </div>
            ...
            <div className="text-center">
              {!showResult ? (
                ...
                >
                  {currentQuestion + 1 < questions.length ? '➡️ Question suivante' : '🏁 Voir les résultats'}
                </button>
              )}
            </div>
          </div>
