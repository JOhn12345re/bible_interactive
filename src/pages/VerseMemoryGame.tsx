...
const VerseMemoryGame = () => {
  ...
  const checkAnswer = () => {
    ...
    if (gameState.gameMode === 'fill-blanks') {
      ...
      const userWords = normalizeText(gameState.userAnswer)
        .split(' ')
        .filter((word) => word.length > 0);
      const expectedWords = gameState.blankedWords.map((word) =>
        normalizeText(word)
      );
      ...
      // Si pas exactement correct, vérifier la similarité (ultra-tolérant)
        ...
        if (similarity >= 0.3) {
          ...
          for (let i = 0; i < minLength; i++) {
            const wordSim = calculateWordSimilarity(
              userWords[i],
              correctWords[i]
            );
            if (wordSim >= 0.3) {
              // Très tolérant même pour les mots individuels
              wordMatches++;
            }
          }
    ...
    if (isCorrect) {
      ...
      if (gameState.currentVerse) {
        markDone(
          `verse-${gameState.currentVerse.id}`,
          `Verset ${gameState.currentVerse.reference} mémorisé`
        );
      }
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      // Tolérance très élevée aux petites différences
        ...
        if (wordSimilarity >= 0.6) {
          // Très tolérant - accepter même 60% de similarité
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <p
                  className={`text-sm ${contrastHigh ? 'text-contrast-text' : 'opacity-90'}`}
                >
            ...
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  >
                    <li>
                      • Les majuscules et minuscules n'ont pas d'importance
                    </li>
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        {gameState.gameMode === 'fill-blanks' && (
          <div
            className={`text-xl text-center mb-6 p-6 rounded-xl ${
        ...
        {gameState.gameMode === 'word-order' && (
          <>
            ...
            <div
              className={`min-h-[100px] p-4 rounded-xl border-2 border-dashed mb-4 ${
                contrastHigh
            ...
            >
              ...
              <p className="text-lg">
                {gameState.userAnswer || 'Clique sur les mots ci-dessous...'}
              </p>
            </div>
