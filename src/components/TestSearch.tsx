...
const TestSearch: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-4">🧪 Test de recherche de versets</h2>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['Jean 3:16', 'Psaume 23:1', 'Genèse 1:1', 'Matthieu 5:3', 'Romains 8:28'].map((ref) => (
              <button
                key={ref}
        ...
        {verses.length > 0 && !loading && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold">✅ Résultats ({verses.length}) :</h3>
        ...
        {verses.length === 0 && !loading && !error && testReference && (
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            <p className="text-yellow-700 text-sm">⚠️ Aucun résultat trouvé pour "{testReference}"</p>
          </div>
        )}
      </div>
    </div>
