...
export default function SermonSection() {
  ...
  useEffect(() => {
    const loadSermons = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : "Erreur lors du chargement des sermons");
  ...
  const filtered = catalog.items.filter(it => {
    const matchesQuery = [it.title, it.preacher, it.description, it.tags?.join(" ")].join(" ").toLowerCase()
      .includes(query.toLowerCase());
    
    const matchesCategory = category === "all" || 
      (category === "sermons" && !it.tags?.includes("psaume") && !it.tags?.includes("chant")) ||
      (category === "chants" && (it.tags?.includes("psaume") || it.tags?.includes("chant")));
      ...
      <header className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="text-sm text-gray-500 flex items-center">
            {filtered.length} élément{filtered.length > 1 ? 's' : ''} trouvé{filtered.length > 1 ? 's' : ''}
          </div>
        </div>
      </header>
      ...
      {filtered.length > 0 && (
        <div>
          <h3 className="text-xl font-semibold mb-4">
            {category === "all" ? "Tous les contenus" : 
             category === "sermons" ? "Sermons" : 
             "Chants & Psaumes"}
          </h3>
                  ...
                  {sermon.tags && sermon.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      ...
                      {sermon.tags.length > 3 && (
                        <span className="text-xs text-gray-400">+{sermon.tags.length - 3}</span>
                      )}
                    </div>
                  )}
                </div>
