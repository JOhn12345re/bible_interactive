...
import { ApiResponse, BibleVerse } from '../types';
import { generalRateLimit } from '../middleware/rate-limit';
import { validateVerseParams } from '../middleware/validation';
...
// GET /api/books/:bookId/chapters/:chapter/verses/:verse - Récupère un verset spécifique
router.get('/:bookId/chapters/:chapter/verses/:verse', validateVerseParams, (req: Request, res: Response) => {
  try {
    ...
    if (!bible[bookName][chapter.toString()][verse.toString()]) {
      const errorResponse: ApiResponse<null> = {
        success: false,
        error: 'Verset non trouvé',
        message: `Le verset ${verse} du chapitre ${chapter} du livre ${bookName} n'existe pas`
      };
      return res.status(404).json(errorResponse);
    }
    
    // Créer l'objet verset
    const bibleVerse: BibleVerse = {
      verset: `${bookName} ${chapter}:${verse}`,
      texte: bible[bookName][chapter.toString()][verse.toString()]
    };
    
    const response: ApiResponse<BibleVerse> = {
      success: true,
      data: bibleVerse,
      message: `Verset ${verse} du chapitre ${chapter} du livre ${bookName}`
    };
...
// GET /api/books/:bookId/chapters/:chapter/verses - Récupère tous les versets d'un chapitre
router.get('/:bookId/chapters/:chapter/verses', validateChapterParams, (req: Request, res: Response) => {
  try {
    ...
    // Convertir tous les versets du chapitre
    ...
    const response: ApiResponse<BibleVerse[]> = {
      success: true,
      data: verses,
      message: `Tous les versets du chapitre ${chapter} du livre ${bookName} (${verses.length} versets)`
    };
    ...
    const errorResponse: ApiResponse<null> = {
      success: false,
      error: 'Erreur lors de la récupération des versets',
      message: error instanceof Error ? error.message : 'Erreur inconnue'
    };
